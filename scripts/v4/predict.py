...
from bdpan_shuiyin.v4.dataset import ShuiyinTestDataset

assert len(sys.argv) == 3
src_image_dir = sys.argv[1]
save_dir = sys.argv[2]
chk_path = 'checkpoints/v4/ep352.pd'
step = 700
pad = 34
h_flip_aug = True
v_flip_aug = False
    ...
    for step_i in range(len(steps)):
        ...
        for i in range(0, h, step):
            for j in range(0, w, step):
                ...
                pred_y = model(clip)
                if h_flip_aug:
                    pred_y_h_flip = model(paddle.flip(clip, axis=[3]))
                    pred_y_h_flip = paddle.flip(pred_y_h_flip, axis=[3])
                    pred_y += pred_y_h_flip
                    multi_count += 1
                if v_flip_aug:
                    pred_y_w_flip = model(paddle.flip(clip, axis=[2]))
                    pred_y_w_flip = paddle.flip(pred_y_w_flip, axis=[2])
                    pred_y += pred_y_w_flip
                    multi_count += 1
                if hw_flip_aug:
                    pred_y_hw_flip = model(paddle.flip(clip, axis=[2, 3]))
