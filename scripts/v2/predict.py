...
from bdpan_shuiyin.v2.dataset import ShuiyinTestDataset
from bdpan_shuiyin.v2.psnr_ssim import calculate_psnr, calculate_ssim

# parser = argparse.ArgumentParser(description='predict v1')
...
# parser.add_argument('--out-dir', type=str, required=True, help='out path')
# args = parser.parse_args()

assert len(sys.argv) == 3
src_image_dir = sys.argv[1]
save_dir = sys.argv[2]
chk_path = 'checkpoints/v2/best_epoch_347/model_0.pd'
step = 700
pad = 34
h_flip_aug = True
v_flip_aug = False
    ...
    for step_i in range(len(steps)):
        ...
        for i in range(0, h, step):
            for j in range(0, w, step):
                ...
                _, _, _, pred_y, _ = model(clip)
                if h_flip_aug:
                    _, _, _, pred_y_h_flip, _ = model(paddle.flip(clip, axis=[3]))
                    pred_y_h_flip = paddle.flip(pred_y_h_flip, axis=[3])
                    pred_y += pred_y_h_flip
                    multi_count += 1
                if v_flip_aug:
                    _, _, _, pred_y_w_flip, _ = model(paddle.flip(clip, axis=[2]))
                    pred_y_w_flip = paddle.flip(pred_y_w_flip, axis=[2])
                    pred_y += pred_y_w_flip
                    multi_count += 1
                if hw_flip_aug:
                    _, _, _, pred_y_hw_flip, _ = model(paddle.flip(clip, axis=[2, 3]))
