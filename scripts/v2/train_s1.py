...
from bdpan_shuiyin.v2.sa_aidr import STRAIDR
from bdpan_shuiyin.v2.loss import ShuiyinLoss
from bdpan_shuiyin.v2.optim import CosineAnnealingRestartLR
from bdpan_shuiyin.v2.dataset import ShuiyinTrainDataset
from bdpan_shuiyin.v2.psnr_ssim import calculate_psnr, calculate_ssim

parser = argparse.ArgumentParser(description='train shuiyin v2')
# model config
# data config
...
parser.add_argument('--out-dir', type=str, default='./output_v2', help='out dir')
...
def build_data():
    train_dataset = ShuiyinTrainDataset(
        ...
        hflip_p=0.5, vflip_p=0.0, rotate_p=0.3, color_jit_p=0.5,
        cutout_big_p=0.2, cutout_small_p=0.2, salt_p=0.2, real_p=0.1,
        is_to_tensor=True, use_mem_cache=False, cache_dir='cache_dataset_train', cache_live=[2, 4],
    )
...
def build_model():
    from bdpan_shuiyin.v2.init import init_model
    model = STRAIDR(num_c=128)
    init_model(model)
...
def val_step(trainer: Trainer, bat, bat_idx, global_step):
    ...
    for i in range(0, h, step):
        for j in range(0, w, step):
            ...
            _, _, _, pred_y, pred_mask = model(clip_x)
...
def main():
    ...
    model = build_model()
    loss_func = ShuiyinLoss(mode='l1', weight=[0.4, 0.5, 0.1])
