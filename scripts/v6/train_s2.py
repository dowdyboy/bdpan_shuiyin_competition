...
parser = argparse.ArgumentParser(description='train shuiyin v6 stage 2')
# model config
# data config
...
parser.add_argument('--img-size', type=int, default=768, help='input img size')
# optimizer config
...
parser.add_argument('--out-dir', type=str, default='./output_v6', help='out dir')
...
def build_data():
    train_dataset = ShuiyinTrainDataset(
        ...
        hflip_p=0.5, vflip_p=0.5, rotate_p=0.5, color_jit_p=0.1,
        cutout_big_p=0.1, cutout_small_p=0.1, salt_p=0.1,
        real_p=0.6, logo_p=0.75,
...
def main():
    ...
    model = build_model()
    loss_func = ShuiyinLoss(mode='psnr', weight=[0.55, 0.45, 0.01])
