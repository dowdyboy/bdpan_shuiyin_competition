...
from bdpan_shuiyin.v7.loss import ShuiyinLoss
from bdpan_shuiyin.v7.optim import CosineAnnealingRestartLR
from bdpan_shuiyin.v7.dataset2 import ShuiyinTrainDataset
from bdpan_shuiyin.v7.psnr_ssim import calculate_psnr, calculate_ssim


parser = argparse.ArgumentParser(description='train shuiyin multi scale image')
# model config
parser.add_argument('--model-type', type=str, default='v7', help='model type')
# data config
...
parser.add_argument('--ms', type=int, nargs='+', default=[512, 640, 768, 896, 1024, 1152, 1280, 1536], help='multi scale crop')
# optimizer config
...
parser.add_argument('--out-dir', type=str, default='./output_ms', help='out dir')
...
def build_data():
    train_dataset = ShuiyinTrainDataset(
        ...
        hflip_p=0.5, vflip_p=0.2, rotate_p=0.2, color_jit_p=0.2,
        cutout_big_p=0.1, cutout_small_p=0.1, salt_p=0.1,
        real_p=0.5, logo_p=0.75,
        is_to_tensor=True, use_mem_cache=False, cache_dir='cache_dataset_train_s2', cache_live=[1, 3],
    ...
    val_dataset = ShuiyinTrainDataset(
        ...
        img_size=args.img_size,
        hflip_p=0.0, vflip_p=0.0, rotate_p=0.0, color_jit_p=0.0,
        cutout_big_p=0.0, cutout_small_p=0.0, salt_p=0.0,
        real_p=0.4, logo_p=0.6,
        is_to_tensor=True, use_mem_cache=False, cache_dir='cache_dataset_val_s2', cache_live=[99999, 999999],
        is_val=True, val_ratio=1.0,
    )
...
def build_model():
    assert args.model_type in ['baseline', 'v6', 'v7']
    if args.model_type == 'baseline':
        ...
        init_model(model)
    elif args.model_type == 'v7':
        from bdpan_shuiyin.v7.sa_aidr import STRAIDR
        from bdpan_shuiyin.v7.init import init_model
        model = STRAIDR(
            unet_num_c=[32, 64, 128, 256, 512],
            fine_num_c=[128],
        )
        # init_model(model)
        pre_weight = paddle.load('./output_ms_v7/saved_chkp/best_epoch_326/model_0.pd')
        model.load_dict(pre_weight)
    return model
...
def main():
    ...
    model = build_model()
    loss_func = ShuiyinLoss(mode='psnr', weight=[0.5, 0.5, 0.0])
