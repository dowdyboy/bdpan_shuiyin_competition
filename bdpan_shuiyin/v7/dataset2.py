...
from .watermark_generator import ShuiyinGenerator, ShuiyinRealGenerator, ShuiyinLogoGenerator
...
class ShuiyinTrainDataset(Dataset):
    ...
    def __init__(self, root_dir, img_size,
                 ...
                 real_p=0.1, logo_p=0.5,
                 is_to_tensor=True, is_val=False, val_ratio=0.1,
        ...
        self.generator3 = ShuiyinLogoGenerator(
            logo_dir=os.path.join(self.root_dir, 'watermark_logo'),
        )
        # data aug
    ...
    def _put_cache(self, idx):
        ...
        if random.random() < self.real_p:
            if random.random() < self.logo_p:
                im_gen = self.generator3.generate_shuiyin_image(Image.fromarray(im_bg))
            else:
