...
class ShuiyinLogoGenerator():

    def __init__(self, logo_dir,
                 scale=[0.5, 1.5],
                 rotate=[-30, 30],
                 alpha=[10, 50],
                 ):
        super(ShuiyinLogoGenerator, self).__init__()
    ...
    def _init_generator(self):
        for logo_filename in os.listdir(self.logo_dir):
            logo_path = os.path.join(self.logo_dir, logo_filename)
            logo_im = np.array(Image.open(logo_path))
            # assert len(logo_im.shape) == 3 and logo_im.shape[2] == 4
            if len(logo_im.shape) == 3 and logo_im.shape[2] == 4:
                self.logo_imgs.append(logo_im)

    def generate_shuiyin_image(self, bg_image):
        alpha = random.randint(self.alpha[0], self.alpha[1]) / 100.
        rotate = random.randint(self.rotate[0], self.rotate[1])
        scale = random.random() * (self.scale[1] - self.scale[0]) + self.scale[0]
        idx = random.randint(0, len(self.logo_imgs) - 1)
        im_logo = np.copy(self.logo_imgs[idx])
        # im_logo[:, :, 3] *= int(255 * alpha)
        im_logo[:, :, 3][im_logo[:, :, 3] > 0] = int(255 * alpha)
        im_logo = Image.fromarray(im_logo)
        ori_w, ori_h = im_logo.size
        new_w, new_h = int(ori_w * scale), int(ori_h * scale)
        im_logo = im_logo.resize((new_w, new_h))
        im_logo = im_logo.rotate(rotate, expand=True)
        new_w, new_h = im_logo.size
        if bg_image.mode != 'RGBA':
            bg_image = bg_image.convert('RGBA')
        bg_w, bg_h = bg_image.size
        if new_w > bg_w or new_h > bg_h:
            im_logo = im_logo.resize((bg_w, bg_h))
            pos_x, pos_y = 0, 0
        else:
